...
export default function NewScreen() {
  ...
  const addPlant = usePlantStore((state) => state.addPlant); // Get the addPlant action from the plant store
  ...
  const handleChooseImage = async () => {
    ...
    if (!result.canceled) {
      setImageUri(result.assets[0].uri); // Set the image URI to the first asset in the result
    }
  };
